...
import { RotateCcw, AlertTriangle, Cpu, Activity, Layers, ShieldAlert, BarChart } from "lucide-react";
...
export function ArchitectureResultDisplay({
  ...
  const result = useMemo(() => {
    return deriveArchitecture(context, selections, tradeOffs);
  }, [context, selections, tradeOffs]);

  const componentsByLayer = useMemo(() => {
    const grouped = result.requiredComponents.reduce((acc, comp) => {
    ...
    const order = ["UI", "Application", "Integration", "Data", "Security", "Operations"];
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm font-mono">
            Derived deterministically from {selections.filter(s => s.status === 'IN_SCOPE').length} capabilities.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-primary/10 border-primary/20">
          <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-2">
            ...
            <div className="text-4xl font-bold" data-testid="indicator-complexity">{result.indicators.complexityScore}</div>
            <div className="text-xs uppercase tracking-widest font-semibold text-muted-foreground">Complexity</div>
          </CardContent>
        </Card>
        <Card className="bg-secondary/50">
          <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-2">
            <Activity className="w-8 h-8 text-muted-foreground mb-2" />
            <div className="text-4xl font-bold" data-testid="indicator-operational">{result.indicators.operationalOverheadScore}</div>
            <div className="text-xs uppercase tracking-widest font-semibold text-muted-foreground">Operational Overhead</div>
          </CardContent>
        </Card>
        <Card className="bg-secondary/50">
          <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-2">
            <BarChart className="w-8 h-8 text-muted-foreground mb-2" />
            <div className="text-4xl font-bold" data-testid="indicator-change">{result.indicators.changeCostLaterScore}</div>
            <div className="text-xs uppercase tracking-widest font-semibold text-muted-foreground">Change Cost Later</div>
          </CardContent>
        </Card>
      </div>
            ...
            {result.risks.length === 0 ? (
              ...
              result.risks.map((risk, idx) => {
                ...
                return (
                  <div 
                    key={idx} 
                    className={`p-3 border rounded-md text-sm space-y-1 ${colors[risk.level]}`}
