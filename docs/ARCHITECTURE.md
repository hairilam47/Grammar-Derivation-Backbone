...
| EAStudio Path B Phase 2 — Level of Specification (LoS) framework + L3 generator (Task #114) | The Studio canvas gains a per-lens LoS toggle (L1 Business / L2 Application / L3 Technology) and an L3 surface that mounts a fullscreen 3D structural canvas mechanically derived from each architecture's CTAD selections | New `ACW_LOD_LEVELS` / `AcwLodLevel` / `isAcwLodLevel` / `isAcwLodRangeShape` / `isVisibleAtLod` in `acw/acwGrammar.ts`; optional `lodRange?: readonly [AcwLodLevel, AcwLodLevel]` field on `AcwNode` (read-validator + create-API + defensive shape-check + node allowlist all extended; **schema stays `acw-1.0`** because the field is optional and absent on legacy nodes); `acwLensStructure.enumerateLensVisibility` accepts an optional `currentLodLevel` and filters direct siblings + child-refs through `isVisibleAtLod`; per-lens `activeLodByLens?: Record<string, AcwLodLevel>` slice on view-state with `getActiveLod` (default `2`) / `setActiveLod` accessors (**view-state schema stays `acw-view-1.0`**, top allow-list extended); new `StudioTopBar` LoS button group gated to `lensId === "/workspace/studio"`; `StudioCanvas` mounts `Canvas3DStructural` as a sibling of `.es-shell` inside `position: fixed; inset: 0; z-0` (Track 3 fullscreen parity) when LoS=3 with the topbar lifted to `position: relative; z-index: 10` so the L1/L2/L3 toggle remains reachable, and edge-triggers `runL3GeneratorForPersistedArchitectures` once per session entry into L3 (ref-gated); new isolated module `acw/l3/l3Generator.ts` with a closed mapping table (`(infrastructure, hostingModel="kubernetes") → ComputeNode "Kubernetes cluster"`; any other `hostingModel` value mints nothing — no fallback; `(infrastructure, databaseClass) → Component` labelled with the chosen option value VERBATIM), stable id `l3:<architectureId>:<parentId>:<sectionId>:<paramId>` (the `architectureId` segment prevents cross-architecture collisions when the Studio shell projects multiple persisted architectures into the same workspace) parented to the L2 origin node carrying the `boundParam` reference (no specific container required), per-architecture session memo keyed by a CTAD state hash, mints `lodRange: [3, 3]` so the nodes are visible only at L3, never deletes anything, and does not orphan-recreate when an L2 origin is pruned; two new build-time invariants — `acwL3IsolationInvariants.test-shape.ts` pinning the L3 module's CTAD-store named-import surface to a SINGLE symbol (`exportArchitectureState`) with explicit denylist self-test rejecting `listArchitectures` and re-exported types (`/src/acw/l3/` added to ACW isolation EXCLUDED prefix list) and `acwL3GeneratorInvariants.test-shape.ts` (snapshot-and-restore probes for idempotency, no-orphan-recreate, the `lodRange: [3, 3]` contract, and the closed mapping table including the "On-prem hostingModel mints nothing while Document databaseClass mints 'Document' verbatim" fixture). The Studio shell carries no architectureId mapping, so the entry point reads the CTAD storage key (`ctad.state.v1`) directly through `localStorage` — a global runtime affordance that the import-only L3 isolation invariant does not restrict — and delegates each id to `exportArchitectureState`; the snapshot shape is derived locally via `ReturnType<typeof exportArchitectureState>` so no CTAD type re-export is required. **Deviation**: the original brief described a cascading `deleteNode` in `acwStore`, but the store ships only `deleteEdge`; the orphan-recreate invariant therefore prunes the L2 origin via direct test-fixture storage manipulation and the generator's contract is "no orphan re-mint" rather than the brief's broader "cascade with parent" framing | Reversible by deleting `acw/l3/`, the two App.tsx side-effect imports, the LoS toggle block in `StudioTopBar`, the L3 surface block + the generator-trigger ref + the workspace-tick subscription in `StudioCanvas`, the optional `lodRange` field everywhere it was added (interface, allowlist, validator branch, createNode shape-check, conditional spread), the `currentLodLevel` parameter on `enumerateLensVisibility`, the `activeLodByLens` slice on view-state (allow-list entry + emptyView + normalize + validator branch + accessors), and the `ACW_LOD_LEVELS` / `AcwLodLevel` / `isAcwLodLevel` / `isAcwLodRangeShape` / `isVisibleAtLod` exports from `acwGrammar.ts`; persisted workspaces and view-state documents continue to load because no schema field was made required |
| EAStudio Path B Phase 3 — right-click technology swap menu + Organisational Unit (OU) overlay (Task #115) | The Studio canvas gains a right-click "Swap technology" popover on every node card and a categorical Organisational Unit overlay toggled from the topbar | New isolated registry `acw/orgUnits/ouStore.ts` (key `acw.organisational-units.v1`, schema `ou-1.0`, allow-listed `OuDoc { schemaVersion, units: OrgUnit[] }` with `OrgUnit { id, name, parentId? }` — duplicate ids, empty ids/names, and dangling parentIds all REJECTed by the read-validator); idempotent `createOu` (re-using a supplied id is a no-op — note the **intentional contract drift**: the brief sketched `createOu(entry: OuEntry)` with a required caller-supplied slug, but `createOu` ships `id` as OPTIONAL and falls through to a `freshOuId()` auto-generate when omitted, because the only first-party caller — `NodePropertiesPanel`'s "Add unit" `window.prompt` affordance — has nowhere to source a slug; the read-validator rejects empty / duplicate ids regardless of origin, and programmatic callers that DO want a stable slug can still pass `id` and the idempotent re-use path makes that round-trip safe), `updateOuName`, and a **transactional** `removeOu` (the cascade-clear runs FIRST through the validator-gated `updateNodeProperties` for every node carrying the target id; if any clear is refused the OU document is NOT mutated and the verbatim refusal reason surfaces back, so a removed unit can never leave a node carrying a dangling OU id; on success the cascade also reassigns child units' `parentId` to `undefined` so the registry never points at a missing parent); new `acw/orgUnits/ouHue.ts` with `OU_HUE_SATURATION_PERCENT = 35` / `OU_HUE_LIGHTNESS_PERCENT = 22` and a deterministic djb2 `hashHueForOu(id)` → integer in `[0, 359]` (categorical-only — continuous / sequential / value-encoded ramps are forbidden by the brief); `AcwNode` widened with optional `organisationalUnitId?: string` (read-validator + `CreateNodeRequest` + `UpdateNodePropertiesRequest` extended; **schema stays `acw-1.0`** because the field is optional and absent on legacy nodes; the same `updateNodeProperties` change also fixed a pre-existing bug where `lodRange` was dropped from the spread; the read-validator additionally REFUSES `organisationalUnitId` on any node where `isDomainContainer === true` — sealed containers cannot carry an OU binding — which is an **intentional product-policy add** beyond the brief's purely-additive framing, motivated by the same "containers are organisational scaffolding, not assignable units" stance the seal already encodes elsewhere); per-lens `showOrgOverlayByLens?: Record<string, boolean>` slice on view-state with `getShowOrgOverlay` / `setShowOrgOverlay` accessors (**view-state schema stays `acw-view-1.0`**, top allow-list extended); new `components/acw/studio/NodeContextMenu.tsx` (presentational leaf — lists the bound CTAD param's alternative options via `resolveBoundOptions`, marks the current selection as disabled, filters the current selection out of the actionable list (surfacing it once via a header annotation `current: <value>`) so the `"No other options available."` empty state fires whenever the bound parameter has exactly one option, intercepts the native context menu only on nodes that carry a `boundParam` (unbound nodes release the browser default), and routes every accepted swap through `updateNodeBinding` with the existing `boundParam` `sectionId` / `paramId` and the chosen `optionValue` so swaps inherit Phase-5 validator gating, then on success calls `invalidateL3Memo()` and — when `activeLod === 3` — re-runs `runL3GeneratorForPersistedArchitectures()` so the L3 surface refreshes inside the same gesture rather than on the next workspace touch); the Studio `NodeCard` in `DomainGrid.tsx` derives its rendered label and icon from `resolveLabel(node)` / `resolveIcon(node)` (mirroring `InteractiveCanvas2D`'s Phase-5 wiring) with a fall-through to the palette tile's icon when `boundTechnologyCategory` is unset, so a swap surfaces on the canvas in the same gesture without a re-mount; `DomainGrid` hosts the menu state at the grid root, wires `onContextMenu` on every `NodeCard`, and applies `backgroundColor: cssForOu(id)` plus an aria-label augmentation `"<label> (organisational unit: <unit name>)"` whenever the per-lens overlay slice is ON and the node carries a resolvable OU id (dangling ids silently skip tinting); `NodePropertiesPanel` gains an "Organisational unit" dropdown with a `"Not specified"` sentinel that clears the field and registry options sorted alphabetically by name (locale-aware, case-insensitive) + Add unit (window.prompt → `createOu` → auto-bind) + Remove (window.confirm → `removeOu`, disabled unless the node currently carries an OU id); `StudioTopBar` gains an "Org View" toggle gated to `lensId === "/workspace/studio"` (Lucide `Users` icon, `aria-pressed` mirrors the slice); two new build-time invariants — `acw/orgUnits/ouStoreInvariants.test-shape.ts` (schema lock; validator negatives for unknown schemaVersion / unknown top fields / duplicate ids / empty ids / dangling parentIds; idempotent `createOu`; cascade-clear on `removeOu`; parent-reassign on `removeOu`; create-time empty-name and dangling-parent refusals — all using snapshot-and-restore against an ad-hoc sealed container so the user's persisted workspace and OU registry survive a probe failure) and `acw/orgUnits/ouHueInvariants.test-shape.ts` (S=35% / L=22% constants pinned; `hashHueForOu` determinism + range; `cssForOu` formula match; empty-id refusal); a 12j OU round-trip probe in `acwGrammarV2Invariants.test-shape.ts` covers set / preserve-through-binding / clear / empty-string-refusal at the v2 grammar layer, and a sibling 12k swap-visibility probe seeds a node with `boundParam.optionValue: "Kubernetes"`, calls `updateNodeBinding` to swap to `"Nomad"`, and asserts `resolveLabel` returns `"Nomad"` against the live workspace snapshot — locking the binding-read contract end-to-end so a future renderer change cannot silently revert to the pre-Phase-3 `node.label` rendering and break the swap-visibility acceptance criterion; both new invariant modules are side-effect imported in `pages/acw/WorkspaceShell.tsx`. The categorical-only colour scheme is the structural enforcement of the brief's prohibition on continuous / sequential / value-encoded ramps. ACW isolation allow-list unchanged (`@/acw` permitted prefix already covers `@/acw/orgUnits/*`); refusal channel unchanged | Reversible by deleting `acw/orgUnits/`, removing `organisationalUnitId` from `AcwNode` / `CreateNodeRequest` / `UpdateNodePropertiesRequest` (including the sealed-container refusal branch in the read-validator) (and probes (12j) and (12k) in `acwGrammarV2Invariants.test-shape.ts`), removing the `showOrgOverlayByLens` slice from view-state, deleting `NodeContextMenu.tsx`, reverting the swap path's `invalidateL3Memo()` + L3 re-run hook, reverting the Studio `NodeCard` in `DomainGrid` to render the palette-tile icon and `node.label` directly (dropping the `resolveLabel` / `resolveIcon` wiring) and dropping the menu state, `onContextMenu`, `overlayCss`, `ouName`, and `data-ou-*` plumbing, reverting `NodePropertiesPanel` (drop `FIELD_ORG_UNIT` / `ADD_UNIT_LABEL` / `REMOVE_LABEL` constants, the OU subscription, `commitOrgUnit` / `onAddUnit` / `onRemoveUnit`, the OU field block), reverting `StudioTopBar` (drop `ORG_VIEW_LABEL` / `ORG_VIEW_TITLE`, the `showOrgToggle` branch and the `Users` import), and removing the two side-effect imports in `WorkspaceShell.tsx` |
...
- **L3 Technology** — fullscreen 3D structural canvas. The L3
  ...
  L2 origin node that carries the corresponding `boundParam`
  reference (no specific container is required).
...
- The L3 generator is **pure, idempotent, deterministic** and
  carries a stable id
  `l3:<architectureId>:<parentId>:<sectionId>:<paramId>` (where
  `parentId` is the L2 origin node carrying the `boundParam`
  reference). The `architectureId` segment is required because
  the Studio shell may project multiple persisted architectures
  into the same ACW workspace; without the discriminator a
  second architecture's mints would collide silently with the
  first's at any shared (parent, section, param) tuple. Re-runs
  short-circuit through the store's id-collision rule.
  Re-running with no CTAD change is a byte-identical no-op
  (verified by `acwL3GeneratorInvariants.test-shape.ts`).
- The generator never deletes anything, every minted node
  carries `lodRange: [3, 3]` (so L1 / L2 stay pixel-identical to
  pre-Phase-2 behaviour), and the generator does not require
  any specific parent container — it walks the existing L2
  nodes that carry a `boundParam` reference and parents the
  minted child to that origin. If the origin is later pruned
  the child is not orphan-recreated on re-entry.
- The L3 module is the only file under `/src/acw/l3/` permitted
  to touch the CTAD store. The dedicated isolation invariant
  (`acwL3IsolationInvariants.test-shape.ts`) pins the
  CTAD-store named-import surface to a SINGLE symbol —
  `exportArchitectureState` — and explicitly rejects every
  other named import (including `listArchitectures` and any
  re-exported type), as well as namespace and dynamic
  imports. The Studio shell carries no architectureId mapping,
  so the generator's `runL3GeneratorForPersistedArchitectures`
  entry point enumerates the architecture roster by reading
  the CTAD storage key (`ctad.state.v1`) directly through
  `localStorage` — a global runtime affordance that the
  import-only L3 isolation invariant does not restrict — and
  hands each id to the allowlisted `exportArchitectureState`
  for the snapshot. The snapshot shape is derived locally via
  `ReturnType<typeof exportArchitectureState>` so no CTAD
  type re-export is required.
...
| CTAD section | CTAD parameter | Triggering value | ACW element type | Label |
| --- | --- | --- | --- | --- |
| `infrastructure` | `hostingModel` | `"kubernetes"` only | `ComputeNode` | `"Kubernetes cluster"` (literal) |
| `infrastructure` | `hostingModel` | any other value | — | nothing minted (no fallback) |
| `infrastructure` | `databaseClass` | any registry option | `Component` | the chosen option value VERBATIM (no suffix, no transform) |
...
- `StudioCanvas` mounts `Canvas3DStructural` as a sibling of
  `.es-shell` inside `position: fixed; inset: 0; z-0` (Track 3
  fullscreen parity) when LoS=3; the body of `.es-shell` is
  suppressed in this branch. The `StudioTopBar` is lifted to
  `position: relative; z-index: 10` so the L1/L2/L3 toggle
  remains interactive on top of the fixed canvas — it is the
  only above-canvas chrome at L3, and clicking L1 or L2 (or
  pressing Escape) is how the user leaves L3.
- The L3 generator is edge-triggered once per session entry
  into L3 (`activeLod !== 3` → `activeLod === 3`) by a
  ref-gated effect in `StudioCanvas` that calls
  `runL3GeneratorForPersistedArchitectures`. Re-runs would be
  safe (the generator is constitutionally idempotent and
  carries a per-architecture session memo keyed by a CTAD
  state hash) but the edge-trigger keeps call counts
  predictable. The `clearStudio` action additionally flushes
  the session memo via `__l3GeneratorInternals.resetMemoForTest`
  so a Clear → re-enter L3 cycle re-mints applicable children.
