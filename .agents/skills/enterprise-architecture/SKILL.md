...
- **Cross-layer arrows** use the dotted style (`..>`) to distinguish them from intra-layer arrows in the per-layer diagrams (which use `->` and `-->`). Labels: `realizes`, `implementedBy`, `reads`, `writes`, `deployedTo`. The `usecase --> task` `implementedBy` arrow is computed transitively — drawn when at least one story that realizes the use case shares an `implementedBy` function with the task. Tasks are rendered as nested rectangles inside their owning process container so the arrow terminates at the specific task being implemented.
