...
- **Canvas Enhancements (Task #173):** Four UI-only features added to the EAStudio canvas: (1) Infinite canvas grid — `backgroundPosition` tracks pan offset so the dot grid scrolls with the canvas. (2) Layers — additive optional `layerIds?: readonly string[]` on `AcwNode`; per-lens `layersByLens` slice on `acwViewState` (`AcwLayerDef`, `AcwLensLayerSlice`, helpers: `addLensLayer`, `renameLensLayer`, `deleteLensLayer`, `toggleLensLayerVisibility`, `getLensLayers`, `getLensLayerVisibility`); `LayersPanel.tsx` side panel with add/rename/delete/visibility-toggle; `NodePropertiesPanel` gains a layer multi-select; `acwLensStructure.ts` 6th param `activeLayerIds` filters out hidden-layer nodes. (3) Focus Mode — module-level `focusStackByLens` Map in `InteractiveCanvas2D`; F key on non-empty selection pushes focused node ids onto the stack (falling back to recentre when selection is empty); Escape pops the stack when L3 and connect mode are both inactive; a floating "Focused on N nodes / Show all" bar appears while in focus mode. (4) Standalone Tab — `?standalone=1` query param in `StudioCanvas` passes `hideLensNav` to `WorkspaceShell`, suppressing the lens switcher; StudioTopBar gains a "New tab" button (ExternalLink icon) that opens the page with that param. No new stores; no schema bumps to `acw-1.0` / `acw-view-1.0`.
