...
- **`/workspace/*` (ACW)** — TOGAF-aligned workspace shell with five lens views (Context & Domain, System Landscape, Integration, Deployment & Infrastructure, Operations & Continuity) **plus a sixth EAStudio canvas lens at `/workspace/studio` (Task #90 Phase 1, Task #91 Phase 2)** and reusable 2D and 3D canvas primitives. Build-time isolation invariant — imports nothing from the decision pipeline.
  ...
  - **EAStudio Phase 2 (Task #91).** Layered interactive editing on the four-domain canvas, additive on top of Phase 1; schema stays `acw-1.0`. The store gains five strictly-optional descriptive node fields (`description`, `owner`, `status`, `maturity`, `priority`) with closed enums (`status: planned|active|deprecated`, `maturity: initial|managed|defined|quantitatively-managed|optimizing`, `priority: low|medium|high|critical`); the read-validator accepts each enum's legal values and rejects unknown literals, empty-string text, and wrong primitive types. Two new validator-gated mutations route every property change through the same surface as the rest of ACW: `updateNodeProperties(nodeId, patch)` (text and enums; `null` removes the field) and `renameNode(nodeId, label)`; both refuse for the four sealed domain containers via the existing `updateNodeBinding` discipline, with the refusal text emitted verbatim by the validator (`canCreateEdge` was extended to refuse a CONNECTS attempt with a sealed `fromId` or `toId` via a new `ValidatorWorkspaceView.isSealed` predicate, so the store no longer ships a parallel hand-crafted refusal string). `updateNodeBinding` was hardened to spread the prior node so additive optional fields (`domainTag`, plus the five Phase 2 fields) are never silently dropped during a binding update — the previous reconstruct-from-subset implementation would clobber them. The grammar permits `Zone↔Zone` and `Component↔Component` CONNECTS pairs (in addition to the pre-existing `System↔System`); domain containers stay exempt from connect and delete. `acwViewState` gains four lens-keyed visual-only slices on the existing `acw.workspace.view.v1` (no schema bump): `connectModeByLens`, `connectPendingSourceByLens`, `selectedNodeIdByLens`, `selectedEdgeIdByLens`. The Studio surface adds three small components, all built from `@/components/ui` primitives and `lucide-react` icons: a top-right `ConnectToggle` (Design ↔ Connect), a bottom `StatusBar` (live node / connections counts, current `Mode: Design | Connect` label with `pending` sub-state, neutral framework badge), and a right-side `NodePropertiesPanel` (label rename + the five descriptive fields with debounced commits — typing schedules a validator-gated commit ~350ms after the last keystroke; closed-enum dropdowns use `radio-group`-style writes; clearing happens by emptying the field; the panel also lists incident CONNECTS edges with a per-edge delete). Connect-mode dispatch is click-source-then-click-destination through a transparent header-strip overlay on every non-sealed container; clicking the same node twice cancels. `Escape` clears the pending source and exits Connect mode for the active page lens. `InteractiveCanvas2D` renders edges as cubic Bezier curves with arrowheads and a fat invisible hit-line; clicking the path selects the edge in the lens slice, and a small in-canvas confirm pill (Delete / Cancel) appears next to the selected edge so destructive removal still requires a deliberate second click. Right-clicking the path opens the same confirm pill at the click site. Display labels remap on the surface only — schema names are unchanged: `owner → Maintainer`, `priority → Tier`, and tier values render as `Routine / Standard / Elevated / Acute`. Phase 2 invariant probes (10) (optional fields), (11) (view-state slices), and (12a–i) (live-store mutations including sealed-CONNECTS refusal in source / destination / sealed-to-sealed roles, same-node cancel, and full binding preservation across `updateNodeBinding`) live in `acwGrammarV2Invariants.test-shape.ts`. The ACW isolation invariant is intact, the Phase 1 surface is undisturbed, and Phase 2 is documented as strictly removable in `docs/ARCHITECTURE.md` §20A.
