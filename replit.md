...
- **Stepwise authoring wizard:** `components/acw/AuthoringPanel.tsx` is now two 3-step card-based wizards instead of two flat dropdown forms — *Add element* walks Type → Parent → Label and *Add relationship* walks Kind → From → Destination, with a step-dot indicator, Continue/Back navigation gated by an explicit selection, and Parent snap-back when Type changes. The *Bound parameters* subsection renders option pills (including a `Not specified` pill) per bound node. Submission still routes through `createNode` / `createEdge` / `updateNodeBinding`; refusals continue to surface in the shared `acw-refusal-banner`. Constitutional rules are preserved: `lucide-react` only, no traffic lights, no judgement animation, no emoji, every new visible static label asserted via `assertAllAcwPlaceholderLanguage`. See `docs/ARCHITECTURE.md` §18A for full details.
