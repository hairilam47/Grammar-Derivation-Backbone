...
- **EAStudio Path B Phase 2 — Level of Specification (LoS) framework + L3 generator (Task #114):** The Studio canvas gains a per-lens LoS toggle (L1 Business / L2 Application / L3 Technology) on `StudioTopBar` (gated to `lensId === "/workspace/studio"`); selecting L3 mounts `Canvas3DStructural` inside `position: fixed; inset: 0; z-0` (Track 3 fullscreen parity) and runs the L3 generator once on each "edge" entry. The generator (`acw/l3/l3Generator.ts`) is pure / idempotent / deterministic, mints nodes with stable id `l3:<archId>:<sectionId>:<paramId>` and `lodRange: [3, 3]` so they are visible only at L3, and projects each CTAD architecture's hosting + database choices through a closed mapping table (`infrastructure.hostingModel → ComputeNode`, `infrastructure.databaseClass → Component`). The generator no-ops if the sealed `domain-technology` container is missing (no orphan recreate) and never deletes anything. **ACW schema stays `acw-1.0`** and **view-state schema stays `acw-view-1.0`** — both new fields (`AcwNode.lodRange?` and view-state `activeLodByLens?`) are optional and absent on legacy documents, so persisted data loads without migration. Two new build-time invariants enforce the contract: `acwL3IsolationInvariants.test-shape.ts` pins the L3 module's CTAD imports to a closed allowlist (`exportArchitectureState`, `listArchitectures`, plus the matching read-only types), and `acwL3GeneratorInvariants.test-shape.ts` runs snapshot-and-restore probes for idempotency, no-orphan-recreate, and the `lodRange: [3, 3]` contract. The CTAD-import allowlist widens the spec's single-import wording by one helper + types because the singleton acwStore at `/workspace/studio` carries no architectureId mapping — the generator must enumerate the architecture roster itself. See `docs/ARCHITECTURE.md` §20E (and §2A row) for full details.
