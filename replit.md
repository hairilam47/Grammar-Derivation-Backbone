...
- **Phase 3 — Multi-Diagram Logical Design:** A standalone authoring surface at `/ctad/design` for the five logical-layer diagrams (BPMN, ERD, DDL, Sequence, Class). Logical nodes persist into the existing ACW workspace (`acw.workspace.v1`, schema `acw-1.0`) via additively-widened optional fields on `AcwNode`/`AcwEdge` (`diagramType`, `diagramSubtype`, `boundRequirementIds`, `moduleId`, `logicalPosition`, `logicalParentId`, `logicalStyle`). The shell provides a palette/canvas/properties three-column layout, drag-to-create with `diagramType+subtype` stamped on `createNode`, drag-to-reposition via `updateNodePosition`, requirement/module binding via read-only catalogs, and a "Promote to EAStudio" panel that uses `updateNodeParent` to move a logical node into one of the four EAStudio domain quadrants (with grammar-refused quadrants greyed out). The surface renders its own minimal canvas (does NOT import `@/components/acw/**`, which remains off the CTAD allowlist) and is gated by build-time invariants: `paletteRegistryInvariants.test-shape.ts` (palette uniqueness, ctad- namespace, root-droppability, per-diagram count) and `ctadIsolationInvariants.test-shape.ts` (allowlist + named-import discipline for the two new governance read stores).
