...
- **EAStudio Visual Theme (Task #99):** `/workspace/studio` aligns visually to `attached_assets/ea_studio_full_platform_*.html`. The change is render-only: `acw-1.0` schema, grammar, validator, refusal channel, and Phase 1–3 functionality are unchanged. Scoped CSS lives under `.eastudio-root` in `artifacts/canvas-ui/src/index.css` using `es-*` class names. Iconography is `lucide-react` only. Sample/Clear/per-card-delete/per-edge-delete are gated through `window.confirm` and route exclusively to validator-gated store mutations (`createNode` / `createEdge` / `deleteEdge` / `deleteNode`); refusals surface verbatim. See `docs/ARCHITECTURE.md` §20C for full details.
